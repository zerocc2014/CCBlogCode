 ...
 *  @param options 传入的是 MACH_SEND_TIMEOUT
 *  @param timeout 传入的是0
 *
 *  @return
 */
...
struct __CFRunLoopMode {
    ...
    __CFPortSet _portSet;                           // 保存所有需要监听的 port，比如 _wakeUpPort，_timerPort 都保存在这个数组中
...
struct __CFRunLoopTimer {
    ...
    CFMutableSetRef _rlModes;       // 存放所有 包含该 timer 的 mode 的 modeName，意味着一个 timer 可能会在多个mode中存在
...
/**
 通过 ModeName 方式向 RunLoop 对象中添加 一个Mode到commonModes中
